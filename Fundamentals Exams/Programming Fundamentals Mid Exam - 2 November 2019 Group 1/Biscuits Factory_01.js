function biscuitsFactory(arr) {
    let biscuitsProducedPerDayPerWorker = +arr.shift();
    let countOfTheWorkers = +arr.shift();
    let produceCompetingFactoryForMonth = +arr.shift();

    let produceMyFactoryForMonth = (20 * countOfTheWorkers * biscuitsProducedPerDayPerWorker) +
        Math.floor(countOfTheWorkers * 0.75 * biscuitsProducedPerDayPerWorker) * 10;

    let diference = Math.abs(produceMyFactoryForMonth - produceCompetingFactoryForMonth);
    let percentage = diference / produceCompetingFactoryForMonth * 100;
    console.log(`You have produced ${produceMyFactoryForMonth} biscuits for the past month.`);

    if (produceMyFactoryForMonth > produceCompetingFactoryForMonth) {
